...
New remotely uploaded media must begin as `status: processing` with `processingState: quarantined` until its stored bytes, type and dimensions are verified. It cannot be selected in Admin or published while in that state. Move it to `ready` only after verification and record any generated renditions. Failed processing uses `failed`; no public route should expose it.
