...
- At 1280 × 720, the Deszio case-study hero retains a 686 × 429 16:10 frame. At 390 × 844 it recomposes to 358 × 224. The source image computes to `object-fit: contain` inside a restrained 104 by 108 percent parallax layer at both sizes. Its edge content remains visible, mixed aspect ratios receive a dark matte instead of aggressive cropping and both layouts have zero horizontal overflow.
- The first Deszio story image was also inspected at desktop and phone sizes. Its complete composed panel remains visible, the phone parallax advances without exposing the frame and the following story panel enters without clipping.
...
- At 1280 × 720, the migrated local Project editor renders its first ten fields as Title, Slug, Industry, Location, Project duration, Completed on, Platform and stack, About the client, URL and Case study accent colour with zero horizontal overflow. Card theme is absent. The native colour swatch is visible, and entering `#e879a8` in the linked hex input updates the picker to the same value. Every image content block still offers Wide, Full width and Bento feature layouts.
- Migration `0011_project_editor_field_order.sql` leaves 28 Project fields and removes Card theme from the schema, seeded drafts, published data and snapshots.
- A read-only shared Turso verification confirms migrations 0009, 0010 and 0011 are recorded, the same first ten Project fields are active and the obsolete Card theme field count is zero.
- The revised Lenis profile passes numeric wheel and touch compression tests, full source checks and desktop browser stress input. Twelve repeated extreme forward gestures advanced progressively through measured scroll positions from 124 to 877 pixels instead of skipping a chapter. Reverse input regained control without a stuck runtime, the page retained zero horizontal overflow and `data-scroll-runtime` remained ready. Physical-iPhone touch feel remains the final acceptance check.
