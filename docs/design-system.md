...
Tablet and desktop use the persistent GPU particle layer. Phones use lower-density section-owned 2D canvases, but both paths read from the same blue and cyan token palette and follow the same narrative forms. Lenis uses a `0.065` wheel interpolation, `0.86` wheel multiplier and a 72-pixel nonlinear per-event cap. Touch has its own 48-pixel ceiling, `0.05` release interpolation, `1.35` inertia exponent and `0.9` multiplier so fast swipes remain bounded while their release glides. Admin remains native. Route transitions and the public reveal controller are separate from scroll physics.
