...
11. `migrations/0015_project_story_composer.sql` turns the existing Project content blocks field into the unified story order, makes Challenge, Approach and Outcome required when publishing and retains safe defaults for legacy records until their next save.

The SQL is intentionally reviewable and compatible with SQLite or libSQL. Local development uses a file database and Cloudflare Preview and Production use Turso without changing the content model. Foreign keys are enabled, published records must archive before permanent deletion and JSON columns reject invalid documents.
...
Stage 9 connects this schema to Clerk-authenticated Admin handlers for the session, dashboard, collections and item lists. The Cloudflare Worker exposes those protected handlers. The server checks a single approved Clerk user ID and authorised origins. A local bypass exists only when explicitly enabled outside production.
...
Stage 11 adds validated JPEG, PNG, WebP, AVIF and frame-capped GIF images up to 15 MB, validated MP4 video up to 30 MB and a 12,000-pixel dimension limit. Assets retain alt or decorative metadata, focal points, reuse, usage records and draft or published reference protection. Production uploads directly to R2 with short-lived presigned URLs, then verifies and processes role-specific renditions before an asset becomes ready. Local development uses the same asset records with ignored local file storage.

Stage 12 adds flexible Project blocks plus preview, publish, republish, unpublish and archive actions. The unified Project story composer can place the required Challenge, Approach and Outcome entries anywhere between custom blocks. These three entries remain non-removable and store their copy in the existing Project fields. Each entry can show its divider and normal bottom spacing, or remove both together. Project cover, card hover, Image and Bento slots accept either images or validated MP4 videos. Image blocks accept one to three ordered assets and render two assets side by side or three as a responsive group. Bento is a separate block type that accepts multiple assets, reads their stored dimensions and arranges wide, landscape, square, portrait and tall media responsively without replacing them with one precomposed image. Video blocks remain available when one explicit MP4 section is preferred. Every MP4 renders without player controls, muted and looping only while it is in the viewport. Social sharing fields remain image-only because social metadata requires an image URL. Existing single-image blocks and the earlier one-image bento data remain compatible. Publishing reruns required-field, media-readiness and media-type validation and copies the draft to `published_data_json`; public queries never read the working draft. Project lists keep every lifecycle state visible. Draft or archived records can be purged only after reference checks, with snapshots removed transactionally and an audit event retained.
