...
Compact renderers keep those colour and density values attached to each particle identity instead of advancing them with time. Local electron paths, bounded rotation, formation and dispersion continue moving normally, but individual dots no longer brighten, dim or change colour like a twinkle. Desktop renderers retain the slower travelling surface field. `surfaceField.compactFlowScale` owns this distinction.

The central tuning surface is `src/config/experience.ts`. `particleRenderConfig` is the shared renderer contract for glow, pointer motion, surface fields, ambient fields and Home performance profiles. `particleObjectConfig` is the shared recognisable-object contract for Home models and scales, the About Head and portrait, plus Services model settings. Object scale changes target spacing without changing the shader's independent point-size uniform. The Services `anchor` moves the complete desktop or mobile particle stage, while every model has an independent `centreOffset` for correcting its visual centre without changing scale, point size or pointer bulge. Positive X moves right, positive Y moves up and positive Z moves towards the camera. `particleSceneConfig` remains the only place for Home section anchors and formation thresholds. Desktop and tablet retain the persistent GPU narrative while phones retain the section-owned 2D path. Components and shaders must consume these contracts instead of introducing parallel hardcoded settings.

`useParticlePointer`, `updateParticleInteraction` and `particlePointerVertexShaderChunk` form the shared GPU interaction engine. Home, About Head and Services use the same fine-pointer listener, whole-object travel, tilt and local shader bulge. Pointer proximity is measured from each transformed point’s projected screen position, with an aspect-corrected radius, then depth displacement is applied in view space towards the camera. The response therefore follows the surface currently facing the visitor instead of the model’s original local front. Screen radius, view-space depth, point growth, falloff power and strength are all adjustable in `experienceConfig.particles.ambientMotion.pointerBulge`. These controls are independent from object scale. This keeps the interaction feel consistent and prevents route-specific listeners or copied pointer maths from drifting apart.
...
The six supplied Services GLBs are converted by `scripts/optimise-service-models.mjs` into geometry-only point GLBs in `public/models/services`. Each output contains one 4,800-point primitive and no materials, textures, images or animations. The generated files are approximately 58 KB each. The globe allocates 3,744 points to land and 1,056 to ocean so continents remain visibly denser. The Website care target uses the replacement `fita_de_moebius.glb`. Its small particle sphere follows the normalised strip centreline, stays just above the twisting surface and accumulates a rolling quaternion around the local strip-width axis. Runtime profiles select an even subset from these clean targets, keeping one geometry mounted while `targetFrom` and `targetTo` attributes morph on the GPU. Parsed source arrays are cached across responsive count changes, temporary GLTF geometries are disposed after extraction and the Services WebGL canvas is not mounted while the experience is outside its near-viewport boundary.
...
The four mobile target point clouds and their scatter positions are baked into `public/mobile-particles/section-targets.bin`. The binary is about 138 KB and contains 2,200 unique source points per target. The renderer draws those independently sampled positions directly instead of creating jittered copies around a smaller point set, which gives the Webine logo, rings and colony planet a finer and more even particle surface. The hero, Reach and closing point clouds come from their real GLB meshes, while the interlude rings remain procedural. Mobile applies the Reach model's configured rotation to its baked neutral target without loading Three.js, React Three Fiber, GLTFLoader, MeshSurfaceSampler or the WebGL shaders. The scatter projection now uses a 0.22 viewport-relative scale and every point receives an inexpensive two-frequency orbit derived from its baked randomness. The canvases run at the existing 30 FPS ceiling and schedule lightweight ambient motion only while the object is near the viewport.

The interlude-to-process handoff has one renderer-aware control surface at `experienceConfig.particles.processTransition`. `progress` owns the viewport window that converts the live process-line position into intake progress. `gpu` owns whole-field travel, dispersed-field release, gathering stagger and contact fade for the persistent WebGL renderer. `mobile` owns the equivalent contact timing plus scatter radius and curve strength for the section-owned 2D canvas. These values tune timing and shape without changing the shared geometry-derived progress source or introducing callback state. The earlier release of the formed interlude object remains under `particleSceneConfig.interlude.motion.dispersion`.
