...
All recognisable objects now share one USTA-informed electron-motion contract. Stable seeded particle identities receive different offsets, rates, amplitudes, phases and secondary low-frequency paths rather than inheriting only a whole-object rotation. Formed GPU targets keep a porous spatial halo, mobile targets use multi-frequency pixel-space paths and dispersed targets use a wider field. Whole-object float and pointer tilt remain restrained. Scroll rotation is a clamped impulse that eases back to the designed orientation instead of accumulating into an unlimited turn. This keeps the Webine logo, About head, Services orb, Reach form, interlude form and closing planet alive without dissolving their silhouettes. GPU forms also use slow per-point cyan-to-blue colour breathing and bounded opacity pockets, which create uneven USTA-like density without deleting particles or weakening the authored outline.

The central tuning surface is `src/config/experience.ts`. `particles.ambientMotion` owns the shared GPU orientation, local travel, colour range and density contrast. `particles.mobile` owns the phone point size, target looseness, local travel and colour speed. `particles.aboutHead`, `particles.servicesOrb` and `particles.ambientField` own the specialised renderers. `particleSceneConfig` remains the only place for section anchors, scales and formation thresholds. Components and shaders must consume these values instead of introducing parallel hardcoded motion settings.
