...
The central tuning surface is `src/config/experience.ts`. `particleObjectScaleConfig` exposes every recognisable Home, About Head and Services object scale in one block, including separate desktop, tablet and mobile values where the renderer needs them. Object scale changes target spacing without changing the shader's independent point-size uniform. `particles.ambientMotion` owns the shared GPU orientation, local travel and fine-pointer bulge. `particles.surfaceField` owns colour-band speed and scale, residual mixing, density scale and mobile density floors. `particles.mobile` owns the phone point size, target looseness and local travel. `particles.aboutHead`, `particles.servicesMorph` and `particles.ambientField` own specialised density, count and performance limits. The Services `anchor` moves the complete desktop or mobile particle stage, while every model has an independent `centreOffset` for correcting its visual centre without changing scale, point size or pointer bulge. Positive X moves right, positive Y moves up and positive Z moves towards the camera. Every Services model also has its URL and resting angle in `servicesMorph.models`, while its scale points back to `particleObjectScaleConfig`. The Webine logo, Reach rings and closing planet use `heroModel`, `reachModel` and `closingModel` for model URL, sampling size and authored rotation. `particleSceneConfig` remains the only place for Home section anchors and formation thresholds. Components and shaders must consume these values instead of introducing parallel hardcoded settings.
