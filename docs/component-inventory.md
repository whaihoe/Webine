...
| `KineticCursor` | USTA-informed public desktop cursor with separate tight and loose tracking layers | Resting dot and halo, clickable-control morph, 58-pixel circular large-surface state, pressed, hidden, touch-disabled and Admin-excluded |
...
| `PortraitReveal` | Layered colour portrait, SVG-masked grayscale portrait, pointer-local image displacement, rising silhouette particles and editorial team copy | Viewport-triggered bottom-up outline, centrally configurable sequence timing, 595-point mobile and 900-point desktop caps, wide-field origins, independently seeded float, curl and breathing motion, 30 FPS mobile rendering, shared-wrapper vertical parallax, grayscale handoff, displaced liquid ripple, residual trail, local fine-pointer turbulence displacement, touch-safe fallback, 25rem desktop frame cap and oversized indexed names |
| `ServicesChapterController` | Six-card service accordion with one selected model and an independently nullable expanded card | Downward GSAP expansion, semantic triggers, circular large-surface cursor state, persistent collapsed summaries, all-cards-closed state, latest-model retention, keyboard control and responsive flow |
| `ServicesParticleExperience` | Left-anchored sticky Services-only GPU canvas that morphs between six optimised model targets | Responsive stage anchor below navigation, per-model visual-centre offsets, authored resting orientations, screen-space shared pointer engine, scale-independent point and bulge sizing, dispersed GPU transitions, target caching, near-viewport WebGL mounting, loading fallback and centreline Möbius runner |
...
| `useParticlePointer` and `particle-pointer` | Shared GPU object interaction engine used by Home, About Head and Services | One fine-pointer listener contract, screen-space projection, travel, tilt, view-facing local bulge and stable frame-loop state |
...
| `HeroCoverTransition` | Selects the desktop GSAP pin or native coarse-touch sticky cover | Post-layout sibling-ref resolution, production-ready diagnostic state, desktop pin, native sticky and cleanup |
...
| `ProjectCard` | Shared published Project presentation for Works and Home | One token-owned 16:10 media frame across Home and Works, role-aware landing or Works renditions, six-percent movement bleed, priority or lazy loading, restrained GSAP media parallax outside Home, fine-pointer-only hover video playback, lower-edge image gradient with unboxed white label and arrow, large-surface cursor state, clean touch media, one full-content Works link and full runway copy inside Home |
| `MediaLibrary` | Admin upload and reusable asset workspace | Button or drag-and-drop batches of up to 40 files, per-asset alt text or description, caption, decorative state and image focal point, sequential progress, partial-failure retry, 15 MB image and 30 MB MP4 validation, animated GIF preservation, R2 processing state, display name, original filename, reference-aware archive protection and R2 cleanup |
...
| `ItemEditor` | Schema-generated draft and Project workflow | Repeated top and bottom Project actions, save, preview, publish, republish, unpublish, archive, confirmed purge, automatic next Project order, per-Project case-study colour, one-to-three-image blocks, responsive multi-image bento blocks and looping MP4 blocks |
...
| `ProjectStory` and `ProjectStoryBlock` | Public and protected-preview Project storytelling | One shared ordered renderer for movable required Challenge, Approach and Outcome entries, optional custom statement and media blocks, per-entry divider spacing, source-ratio Bento with balanced two-column and three-column packing, dedicated video rendering and safe legacy normalisation |
...
- Services: business-outcome hero and six transparent blurred expanding service cards beside one left-anchored sticky morphing model-particle canvas over a shared ambient field
